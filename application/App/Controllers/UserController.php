...
class UserController extends AbstractTwigController
{
    ...
    public function register_complete(Request $request, Response $response, array $args = []): Response
    {
      ...
      else
      {
        ...
        $register = $this->preferences->APIcall_POST($this->preferences->getApiServer(), $userData, $this->preferences->getApiToken()['access_token'], '/api/v1/users/register');
    ...
    public function login(Request $request, Response $response, array $args = []): Response
    {
      ...
      else
      {
        ...
        $register = $this->preferences->APIcall_POST($this->preferences->getApiServer(), $userData, $this->preferences->getApiToken()['access_token'], '/api/v1/users/' . $userData['username'] . '/password/validate');
        if (isset($register['Message']))
        {
            if ($register['Message'] == "Password Correct") {
              $user_infos = $this->preferences->APIcall_GET($this->preferences->getApiServer(), $this->preferences->getApiToken()['access_token'], '/api/v1/users/' . $userData['username']);

              $userLogin = R::findOne( 'cms_users', 'id = ?', [$user_infos['Id']]);
              if(empty($userLogin) && count($userLogin) == 0) {
                R::exec( 'INSERT INTO cms_users (id, email, points, admin) VALUES (?,?,0,0)',
                [
                  $user_infos['Id'],
                  $user_infos['Email']
                ]);
    ...
    public function my_account(Request $request, Response $response, array $args = []): Response
    {
      if(isset($_SESSION['user_Id'])) {
          ...
          $register = $this->preferences->APIcall_GET($this->preferences->getApiServer(), $this->preferences->getApiToken()['access_token'], '/api/v1/users/' . $_SESSION['user_Id']);
          $players = $this->preferences->APIcall_GET($this->preferences->getApiServer(), $this->preferences->getApiToken()['access_token'], '/api/v1/users/' . $_SESSION['user_Id'] .'/players');
      ...
      if(isset($_POST['code'])) {
        ...
        if( empty($code) ) {
          ...
          if($dedipass->status == 'success') {
            ...
            R::exec( 'UPDATE cms_users SET points=points+? WHERE id=?',
            [
              $virtual_currency,
              $_SESSION['user_Id']
            ]);
            R::exec( 'INSERT INTO cms_points_history (date, user_id, code, points_amount) VALUES (NOW(),?,?,?)',
            [
              $_SESSION['user_Id'],
              $code,
              $virtual_currency
