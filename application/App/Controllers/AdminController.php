...
class AdminController extends AbstractTwigController
{
    ...
    public function news_add(Request $request, Response $response, array $args = []): Response
    {
        ...
        if(isset($_POST['nTitle']) && isset($_POST['nContent'])) {
          ...
          R::exec( 'INSERT INTO cms_news (title, content, author, img_url, date) VALUES (?,?,?,?,NOW())',
          [
            $_POST['nTitle'],
            $_POST['nContent'],
            $_SESSION['user_Name'],
    ...
    public function news_edit(Request $request, Response $response, array $args = []): Response
    {
        ...
        if(isset($_POST['nTitle']) && isset($_POST['nContent'])) {
          //$TryContent = $_POST['nContent'];
          //$_POST  = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
          R::exec( 'UPDATE cms_news SET title=?, content=?, author=?, img_url=? WHERE id = ?',
          [
            $_POST['nTitle'],
            $_POST['nContent'],
            $_SESSION['user_Name'],
    ...
    public function shop_add(Request $request, Response $response, array $args = []): Response
    {
        ...
        else
        {
          ...
          if(isset($_POST['iId']) && isset($_POST['iPrice'])) {
            //$TryContent = $_POST['nContent'];
            //$_POST  = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
            R::exec( 'INSERT INTO cms_shop (id_item, price, forceddescription, quantity, promotion, visible) VALUES (?,?,?,?,?,?)',
            [
              $_POST['iId'],
              $_POST['iPrice'],
              $_POST['iContent'],
    ...
    public function shop_edit(Request $request, Response $response, array $args = []): Response
    {
      ...
      else
      {
        ...
        if(isset($_POST['iId']) && isset($_POST['iPrice'])) {
          //$TryContent = $_POST['nContent'];
          //$_POST  = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
          R::exec( 'UPDATE cms_shop SET id_item=?, price=?, forceddescription=?, quantity=?, promotion=?, visible=? WHERE id = ?',
          [
            $_POST['iId'],
            $_POST['iPrice'],
            $_POST['iContent'],
        ...
        $shopDetail = array();
        if($shopEdit != null) {
          $itemData = $this->preferences->APIcall_GET($this->preferences->getApiServer(), $this->preferences->getApiToken()['access_token'], '/api/v1/gameobjects/item/'.$shopEdit['id_item']);
        ...
        foreach($shopList as $itemShop){
          $itemDataList = $this->preferences->APIcall_GET($this->preferences->getApiServer(), $this->preferences->getApiToken()['access_token'], '/api/v1/gameobjects/item/'.$itemShop['id_item']);
    ...
    public function pages_add(Request $request, Response $response, array $args = []): Response
    {
        ...
        if(isset($_POST['pName']) && isset($_POST['pCat']) && isset($_POST['pContent']) && isset($_POST['pSlug'])) {
          //$TryContent = $_POST['nContent'];
          //$_POST  = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
          R::exec( 'INSERT INTO cms_pages (category, unique_slug, name, content) VALUES (?,?,?,?)',
          [
            $_POST['pCat'],
            $_POST['pSlug'],
            $_POST['pName'],
    ...
    public function pages_edit(Request $request, Response $response, array $args = []): Response
    {
        ...
        if(isset($_POST['pName']) && isset($_POST['pCat']) && isset($_POST['pContent']) && isset($_POST['pSlug'])) {
          //$TryContent = $_POST['nContent'];
          //$_POST  = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
          R::exec( 'UPDATE cms_pages SET category=?, unique_slug=?, name=?, content=? WHERE id = ?',
          [
            $_POST['pCat'],
            $_POST['pSlug'],
            $_POST['pName'],
     ...
     public function settings(Request $request, Response $response, array $args = []): Response
     {
         ...
         if(isset($_POST['settingsSend'])) {
           ...
           foreach($settings as $setting){
             ...
             R::exec( 'UPDATE cms_settings SET value=? WHERE setting = ?',
             [
               $TryContent,
               $setting['setting']
             ]);
      ...
      public function translate(Request $request, Response $response, array $args = []): Response
      {
          ...
          if(isset($_POST['translationSend'])) {
            ...
            foreach($strings as $string){
              ...
              R::exec( 'UPDATE cms_lang SET text=? WHERE str_key = ? AND language = ?',
              [
                $TryContent,
                $string['str_key'],
